...
#define WORKQ_PRIORITY 4 //! For not urgent task
...
void thread0(void)
{
    ...
    while (1)
    {
        ...
        //! Now thread1 finish in half time because it's barely stoped by thread0
        /* Remember to comment out emulate_work(); */
        // emulate_work();
        delta_time = k_uptime_delta(&time_stamp);
        printk("\nthread0 yielding this round in %lld ms\n", delta_time);
        k_msleep(20);
    }
}
...
void thread1(void)
{
    ...
    while (1)
    {
        ...
        emulate_work();
        delta_time = k_uptime_delta(&time_stamp);
        printk("\nthread1 yielding this round in %lld ms\n", delta_time);
        k_msleep(20);
    }
}
